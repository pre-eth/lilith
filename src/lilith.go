package lilith

import (
	"encoding/binary"
	"fmt"
)

type Lilith struct {
	// 	128-bit seed
	seed [8]uint32

	// 	96-bit nonce
	nonce [3]uint32

	// 	Counters for Rabbit PRNG
	ctr [8]uint32

	/*
		Fixed one dimensional representation of the Rijndael S-box shown in img/SRBOX.png
		Rijndael has been designed to be resistant to known cryptanalytic attacks

		This is used in generating a private S-box for the encryption scheme. The choice of
		the S-box provides a much stronger diffusion. Each output bit depends on each input
		bit of the S-box key. In the decryption process, the inverse Sub Bytes transformation
		is applied. The same transformation is implemented but with use of the inverse
		substitution box.

		Since S-box is private, an attacker cannot know the input to the S-box. The 96-bit
		nonce is used in the shuffle process to initialize the S-Bwox
	*/
	sbox [256]byte

	//	Current keystream
	key [8]uint32

	// 	The counter carry bit, phi, which needs to be stored between iterations.
	// 	This counter carry bit is initialized to zero:
	phi byte
}

func (l *Lilith) Init(seed *[16]byte, nonce *[12]byte, operation bool, first_byte byte) {
	// Load seed
	bytesToU32(&l.seed, seed[:])

	// Write bytes to load nonce
	l.nonce[0] = binary.LittleEndian.Uint32(nonce[:4])
	l.nonce[1] = binary.LittleEndian.Uint32(nonce[4:8])
	l.nonce[2] = binary.LittleEndian.Uint32(nonce[8:12])

	// Initial state and counter setup
	state := [8]uint32{}
	setup(&l.seed, &state, &l.ctr)

	// Paper says to call nextState() 3x to finish initialization
	nextState(&state, &l.ctr, &l.phi)
	nextState(&state, &l.ctr, &l.phi)
	nextState(&state, &l.ctr, &l.phi)

	// Get KS0
	extractKeystream(&l.key, &state)

	// Generate the private S-Box
	l.sbox = [256]byte{
		}

	l.sbox = generateSbox(&l.sbox, &l.nonce, operation)

	// Generate the first cipher text
	C0 := [16]byte{}
	generateC0(&C0, &l.key, &l.sbox)

	// Generate KS0
	bytesToU32(&l.key, C0[:])

	//	Get first keystream used for encryption/decryption (KS1)
	nextState(&state, &l.ctr, &l.phi)
	extractKeystream(&l.key, &state)
}

func (l *Lilith) Encrypt(file_bytes []byte) []byte {
	delayedPrint("LILITH "+VersionString+" - ENCRYPT ", TitleColor, textDelay, periodDelay)

	// 	Logic depends on bytes of 16 blocks, so process leftover blocks differently
	//	Extend the output buffer, pad with 0s as needed
	ctextLen := len(file_bytes)
	leftovers := ctextLen & 15
	cutoff := ctextLen
	ctextLen += 16 - leftovers

	for cutoff < ctextLen {
		file_bytes = append(file_bytes, 0)
		cutoff += 1
	}

	i := 0
	cbytes := [8]uint32{}
	for i < ctextLen {
		combiner(&l.key, file_bytes[i:i+16], &l.sbox)
		bytesToU32(&cbytes, file_bytes[i:i+16])
		nextState(&cbytes, &l.ctr, &l.phi)
		extractKeystream(&l.key, &cbytes)
		i += 16
		spinner()
	}

	fmt.Print(" \033[1D\033[m\n\n")

	delayedPrint("Completed encryption.\n", OkColor, textDelay, periodDelay)

	return file_bytes
}

func (l *Lilith) Decrypt(file_bytes []byte) []byte {
	delayedPrint("LILITH "+VersionString+" - DECRYPT ", TitleColor, textDelay, periodDelay)

	i := 0
	ptextLen := len(file_bytes)
	cbytes := [8]uint32{}
	for i < ptextLen {
		bytesToU32(&cbytes, file_bytes[i:i+16])
		invCombiner(&l.key, file_bytes[i:i+16], &l.sbox)
		nextState(&cbytes, &l.ctr, &l.phi)
		extractKeystream(&l.key, &cbytes)
		i += 16
		spinner()
	}

	fmt.Print(" \033[1D\033[m\n\n")

	delayedPrint("Completed decryption.\n", OkColor, textDelay, periodDelay)

	return file_bytes
}
